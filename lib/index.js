...
/** 无更新状态时回退读本地 .git HEAD 的 sha(避免首次检查误报"有新版本")。 */
function localGitSha() {
  try {
    const head = readFileSync(path.join(PLUGIN_DIR, '.git', 'HEAD'), 'utf8').trim()
    const m = head.match(/^ref:\s*(.+)$/)
    if (m) {
      return readFileSync(path.join(PLUGIN_DIR, '.git', m[1].trim()), 'utf8').trim()
    }
    return head
  } catch {
    return null
...
export function apply(ctx) {
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        let current = readUpdateState()
        if (!current) {
          const s = localGitSha()
          if (s) current = { sha: s, source: 'git' }
        }
