...
function ContextRing({ percent }) {
			...
			const color = p >= 90 ? "var(--dsw-alias-state-error-primary)" : p >= 70 ? "var(--dsw-alias-state-warn-primary)" : "var(--dsw-alias-state-success-primary)";
			return react.createElement("div", { style: { position: "relative", width: RING_SIZE, height: RING_SIZE, display: "flex", alignItems: "center", justifyContent: "center" } },
				react.createElement("svg", { width: RING_SIZE, height: RING_SIZE, viewBox: "0 0 " + RING_SIZE + " " + RING_SIZE },
					react.createElement("circle", { cx: RING_SIZE / 2, cy: RING_SIZE / 2, r: r, fill: "none", stroke: "var(--dsw-alias-border-l1)", strokeWidth: 7 }),
					react.createElement("circle", { cx: RING_SIZE / 2, cy: RING_SIZE / 2, r: r, fill: "none", stroke: color, strokeWidth: 7, strokeLinecap: "round", strokeDasharray: String((circ * p) / 100) + " " + String(circ), transform: "rotate(-90 " + RING_SIZE / 2 + " " + RING_SIZE / 2 + ")" })
				),
		...
		// ---- 模型消耗分布:多扇区圆环 + 图例(数据来自 /api/dsh-balance 的 perMessage 明细,按模型聚合) ----
		...
		function ModelDistDonut({ perMessage }) {
			const items = modelDistFromPerMessage(perMessage);
			const total = items.reduce((s, it) => s + it.amount, 0);
			if (items.length === 0 || total <= 0) return null;
			const SIZE = 72, r = 25, circ = 2 * Math.PI * r, cx = SIZE / 2, cy = SIZE / 2;
			const arcs = [];
			let offset = 0;
			items.forEach((it, i) => {
			...
			return react.createElement("div", { style: { width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, borderTop: "1px solid var(--dsw-alias-border-l1)", paddingTop: 6, marginTop: 2 } },
				react.createElement("div", { style: { fontSize: 10, color: "var(--dsw-alias-label-secondary)", fontWeight: 600, letterSpacing: "0.03em" } }, "模型消耗分布"),
		...
		function FloatingWidget(props) {
			...
			react.useEffect(() => {
				...
				if (!dirs || !current) return undefined;
				let dir = null;
				let store = null;
				try {
			...
			const refreshCtx = react.useCallback(() => {
				...
				fetch("/api/dsh-context?sessionId=" + encodeURIComponent(current) + pr + md).then((r) => r.json()).then((res) => {
					if (mySeq === ctxSeqRef.current) setCtxData(res && res.ok ? res : null);
			...
			const refreshBal = react.useCallback(() => {
				...
				fetch("/api/dsh-balance?sessionId=" + encodeURIComponent(current) + pr).then((r) => r.json()).then((res) => {
					if (mySeq !== balSeqRef.current) return;
					setBal(res);
			...
			// 切换会话:重置全部状态(关键:丢弃旧会话的上报 provider,避免带错参数),
			// 随后 host 会按该会话日志实际使用的 provider 判定
			react.useEffect(() => {
				setLiveSel(null);
				setCtxData(null);
				setBal(null);
			...
			cells.push(react.createElement(ContextRing, { key: "ring", percent: ctxData ? ctxData.percent : null }));
			const res = bal;
			if (res && res.ok) {
				cells.push(react.createElement(ModelDistDonut, { key: "mdist", perMessage: res.perMessage }));
		...
		function SelfAddedPluginsTab() {
			...
			cells.push(react.createElement("div", { key: "import-row", style: importRowStyle },
				...
				react.createElement("p", { key: "import-hint", style: importHintStyle }, "选择一个 .zip 插件包安装到本机 dsh,安装后需重启生效")
			));
