 ...
 */
function advanceBalanceCost(state, sessionId, currentTotal, currency, latestMsgId, estH) {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    const attempt = async () => {
      try {
        ...
        const state = readBalanceState()
        advanceBalanceCost(state, sessionId, total, currency, msgId, estH)
        ...
        if (!bound) {
          setTimeout(async () => {
            try {
              ...
              advanceBalanceCost(s2, sessionId, t2, b2.infos[0].currency || DEFAULT_CURRENCY, msgId, estH)
    ...
    handler: async (req, res) => {      try {
        ...
        if (balance.ok && sessionId && balance.infos && balance.infos.length > 0) {
          ...
          if (Number.isFinite(total)) {
            ...
            const state = readBalanceState()
            const r = advanceBalanceCost(state, sessionId, total, currency, cost.latestMsgId, estH)
            writeBalanceState(state)
      ...
      try {
        ...
        if (loader !== undefined && typeof loader.entries === 'function') {
          for (const entry of loader.entries()) {
            ...
            let editable = false
            try {
              const reg = entry.ctx && entry.ctx.registry
              const def = reg && typeof reg.get === 'function' ? reg.get(name) : null
              editable = !!(def && def.Config)
            } catch {
              editable = false
            }
            entries.push({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (!/\.zip$/i.test(fileName)) {
          jsonResp(400, { ok: false, error: '仅支持 .zip 插件包' })
          return
        }
        // base64 预校验(字符集合法,改善坏 zip 的错误提示)
        ...
        const uz = await unzipViaPowershell(ctx, tmpZip, tmpDir)
        if (!uz.ok) {
