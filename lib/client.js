...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---- 悬浮窗主圆环:外圈=上下文占用,内圈=模型消耗分布(同心合并成一个圆环),下方图例 ----
		function ContextRing({ percent, perMessage }) {
			const cx = RING_SIZE / 2;
			// 外圈:上下文占用
			const rOut = 30;
			const circOut = 2 * Math.PI * rOut;
			...
			let offset = 0;
			items.forEach((it, i) => {
				const len = circIn * (it.amount / total);
				arcs.push(react.createElement("circle", {
					...
					strokeWidth: 5,
					strokeDasharray: String(len) + " " + String(circIn),
			...
			return react.createElement("div", { style: { width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 } },
				react.createElement("div", { style: { position: "relative", width: RING_SIZE, height: RING_SIZE, display: "flex", alignItems: "center", justifyContent: "center" } },
					react.createElement("svg", { width: RING_SIZE, height: RING_SIZE, viewBox: "0 0 " + RING_SIZE + " " + RING_SIZE },
						// 外圈底 + 上下文占用
						react.createElement("circle", { cx: cx, cy: cx, r: rOut, fill: "none", stroke: "var(--dsw-alias-border-l1)", strokeWidth: 7 }),
						react.createElement("circle", { cx: cx, cy: cx, r: rOut, fill: "none", stroke: color, strokeWidth: 7, strokeLinecap: "round", strokeDasharray: String((circOut * p) / 100) + " " + String(circOut), transform: "rotate(-90 " + cx + " " + cx + ")" }),
						// 内圈底 + 模型分布扇区
						react.createElement("circle", { cx: cx, cy: cx, r: rIn, fill: "none", stroke: "var(--dsw-alias-border-l1)", strokeWidth: 5 }),
						arcs
					),
